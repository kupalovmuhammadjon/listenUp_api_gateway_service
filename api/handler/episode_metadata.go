...
func (h *Handler) GetTrendingPodcasts(c *gin.Context) {
	...
	if err != nil {
		c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{"error": errors.Wrap(err, "failed to get trending podcasts").Error()})
		log.Println(err)
		return
	}
...
func (h *Handler) GetRecommendedPodcasts(c *gin.Context) {
	...
	if err != nil {
		c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{"error": errors.Wrap(err, "failed to get recommended podcasts").Error()})
		log.Println(err)
		return
	}
...
func (h *Handler) GetPodcastsByGenre(c *gin.Context) {
	...
	if err != nil {
		c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{"error": errors.Wrap(err, "failed to get podcasts by genre").Error()})
		log.Println(err)
		return
	}
...
func (h *Handler) SearchPodcast(c *gin.Context) {
	...
	if err != nil {
		c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{"error": errors.Wrap(err, "failed to find episode").Error()})
		log.Println(err)
		return
	}
